...
where
	...
	and
	MerchantNo not in (select MerchantNo from #SpecialMerchants);
...
where
	c.MerchantNo not in (select MerchantNo from #SpecialMerchants);
...
where
	p.MerchantNo not in (select MerchantNo from #SpecialMerchants)
union all
select
	t.ChannelNo as Gate,
	t.MerchantNo,
...
where
	...
	and
	t.MerchantNo not in (select MerchantNo from #SpecialMerchants)
union all
select
	u.GateNo as Gate,
...
where
	ISNULL(r.CpMerNo, u.MerchantNo) not in (select MerchantNo from #SpecialMerchants);
