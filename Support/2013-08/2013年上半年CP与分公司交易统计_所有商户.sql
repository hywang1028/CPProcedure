...
--4.13.1 标记#AllTrans中“B2C_预授权”
update
	a
set
	a.CategoryName = N'B2C_预授权'
from
	#AllTrans a
where
	a.CategoryName = N''
	and
	a.Gate in (select GateNo from Table_GateCategory where GateCategory1 = N'MOTO');
	
--4.13.2 标记#AllTrans中“B2C_信用支付”
update
	a
set
	a.CategoryName = N'B2C_信用支付'
from
	#AllTrans a
where
	a.CategoryName = N''
	and
	a.Gate in ('5601','5602','5603');
