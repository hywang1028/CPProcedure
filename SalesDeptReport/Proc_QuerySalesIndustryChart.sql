if OBJECT_ID(N'Proc_QuerySalesIndustryChart', N'P') is not null
begin
	drop procedure Proc_QuerySalesIndustryChart;
end
go

create procedure Proc_QuerySalesIndustryChart
...
begin
	raiserror(N'Input params cannot be empty in Proc_QuerySalesIndustryChart', 16, 1);
end
...
select 
	Sales.IndustryName,
	100*SUM(Convert(decimal,ISNULL(Curr.CurrSucceedAmount,0)))/@TotalAmount AmountRatio,
	100*SUM(Convert(decimal,ISNULL(Curr.CurrSucceedCount,0)))/@TotalCount CountRatio,
...
group by 
	Sales.IndustryName
having
	100*SUM(Convert(decimal,ISNULL(Curr.CurrSucceedAmount,0)))/@TotalAmount > 0.01
order by
	CurrAmount;
