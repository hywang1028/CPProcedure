if OBJECT_ID(N'Proc_QueryCPSalesTransReport', N'P') is not null
begin
	drop procedure Proc_QueryCPSalesTransReport;
end
go
...
select 
	...
	ISNULL(Prev.PrevSucceedCount,0) PrevSucceedCount,
	Convert(decimal,ISNULL(Prev.PrevSucceedAmount,0))/100 PrevSucceedAmount,
	ISNULL(LastYear.LastYearSucceedCount,0) LastYearSucceedCount,
	Convert(decimal,ISNULL(LastYear.LastYearSucceedAmount,0))/100 LastYearSucceedAmount,
	
	(ISNULL(Curr.CurrSucceedCount,0) - ISNULL(Prev.PrevSucceedCount,0)) SeqCountIncreasement,
	Convert(decimal,(ISNULL(Curr.CurrSucceedAmount,0) - ISNULL(Prev.PrevSucceedAmount,0)))/100 SeqAmountIncreasement,
	(ISNULL(Curr.CurrSucceedCount,0) - ISNULL(LastYear.LastYearSucceedCount,0)) YOYCountIncreasement,
