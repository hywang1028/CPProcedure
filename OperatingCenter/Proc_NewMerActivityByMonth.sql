...
if OBJECT_ID(N'Proc_NewMerActivityByMonth',N'P') is not null
begin
	drop procedure Proc_NewMerActivityByMonth
end
go

create procedure Proc_NewMerActivityByMonth
...
select
	mers.MerchantType,
	COUNT(mers.MerchantNo) as OpenAccountCnt,
	p.YearMonth,
	sum(case when ISNULL(ad.TransCnt, 0) = 0 then 1 else 0 end) as Level1,
	sum(case when ISNULL(ad.TransCnt, 0) > 0 and ISNULL(ad.TransCnt,0) < 55 then 1 else 0 end) as Level2,
	SUM(case when ISNULL(ad.TransCnt, 0) >= 55 then 1 else 0 end) as Level3
from
